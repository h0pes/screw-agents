...
**Phase 4 gate:** the `blocker` count is now 0. Phase 4 step 4.0 (D-01 Rust benchmark corpus) is active on branch `d01-rust-benchmark` — see `docs/PROJECT_STATUS.md` §"Phase 4 Prerequisites (hard gates)" and `docs/PHASE_4_D01_PLAN.md`. With T-SCAN-REFACTOR shipped, the per-agent autoresearch surface (`scan_agents([single_name])`) is ready for Phase 4 consumption.
