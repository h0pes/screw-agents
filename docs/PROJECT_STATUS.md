...
- Phase 5 challenger package has provider-neutral config/model contracts in
  ...
  `src/screw_agents/challenger/orchestrator.py`. MCP, runner factory/live
  mode wiring, and output integration are still pending.
...
1. **Phase 5 — Multi-LLM challenger.** Add runner factory/live mode wiring so
   configured Claude/Codex CLI runners can be attached to the fixture-backed
   required-mode orchestrator for Claude primary/Codex challenger, Codex
   primary/Claude challenger, and parallel independent review with
