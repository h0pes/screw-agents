...
- Phase 5 challenger package has provider-neutral config/model contracts in
  ...
  wiring is in `src/screw_agents/challenger/runner_factory.py`. MCP/user-facing
  execution commands and output integration are still pending.
...
1. **Phase 5 — Multi-LLM challenger.** Add an explicit user-facing MCP/CLI
   execution surface for configured challenger modes, using the runner factory
   to attach Claude/Codex CLI runners to the required-mode orchestrator.
