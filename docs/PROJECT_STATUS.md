...
- Phase 5 challenger package has provider-neutral config/model contracts in
  ...
  configured challenger mode after merge/exclusion filtering; Claude plugin UX
  for scan-command challenger activation and API/local provider execution are
  still pending.
...
1. **Phase 5 — Multi-LLM challenger.** Extend the explicit
   `finalize_scan_results` challenger attachment with Claude plugin command UX
   for `/screw:scan`, using the runner factory to attach Claude/Codex CLI
   runners to the required-mode orchestrator and the existing output formatters
   to preserve challenger metadata.
