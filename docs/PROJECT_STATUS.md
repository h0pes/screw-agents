...
rerun: 2 are likely residual-risk / incomplete-fix evidence
(`addressController.php:87`, `administrationController.php:129`), 3 are
fix-semantics ambiguous second-order findings
(`administrationController.php:184`, `:211`, `:221`), and 3 are
weak/speculative patched reports (`addressController.php:146`, `:152`,
`administrationController.php:227`). Keep `sqli.yaml` unchanged; this slice is
evidence that MoreFixes patched labels can mix real residual risk, ambiguity,
and ordinary false positives.
...
is refined to v1.0.2 for that pattern. Fs-git CmdI review accepted a reusable
argv precision refinement for `cmdi.yaml` v1.0.2: after a patch moves from
shell `exec` to non-shell `execFile("git", argv)`, CWE-88 reports must identify
a dangerous option accepted at the user-controlled argument position. A fixed
...
Focused CmdI/fs-git v1.0.2 validation at
`/tmp/screw-d02-ossf-fsgit-cmdi-v102-run`, benchmark `20260502-121340`,
completed 2/2 invocations with no executor issues. It kept vulnerable
detection strong with six CWE-78 shell-injection findings and reduced patched
findings from three to zero. The failure-input generator wrote no payloads
because there were no concrete misses or patched false positives. The evaluator
still reports extra unmatched vulnerable findings against OSSF's single truth
span, so treat the remaining FP count as scoring granularity rather than
patched precision noise.
...
- Phase 5 challenger package has provider-neutral config/model contracts in
  ...
  plus fixture, generic CLI, Claude CLI, and Codex CLI runners are in
  `src/screw_agents/challenger/providers.py`.
  ...
  `screw-agents challenger-dry-run`, `screw-agents challenger-run`, and MCP
  ...
  `--challenger <mode> --challenger-execution dry_run|cli` flags; the shared
  `plugins/screw` package now carries both Claude Code and Codex plugin
  metadata. API/local provider execution is still pending. Provider-neutral primary scan contracts,
  ...
  runners include production Claude/Codex output normalization for the live
  envelopes observed during validation. Fixture-mode manual validation is
  ...
  implemented with fixture coverage and live CLI validation for both
  Claude/Codex directions; challenger CLI runners receive the finalized
  findings payload and primary/challenger modes execute only the configured
  challenger participant during review. Backend
  ...
  parallel validation and additional provider adapters remain pending
...
- Remaining CWE-1400 agents beyond `sqli`, `cmdi`, `ssti`, and `xss` are not
...
| Phase 5 | Multi-LLM Challenger System | In progress — challenger config/model contracts, reconciliation, provider runner interface, fixture/generic/Claude/Codex CLI runners, required-mode orchestration, CLI/MCP challenger execution surfaces, report integration, finalize-time attachment, `/screw:scan` challenger flags, primary scan contracts, fixture validation, scan input assembly, backend primary CLI runners, production Claude/Codex primary output normalization, `provider-scan`, MCP `run_provider_scan`, optional provider-scan finalization, backend composed primary-plus-challenger workflow, backend parallel primary reconciliation, universal `/screw:scan` provider-primary/parallel flags, repo-local Codex plugin metadata, and Codex skills for command-equivalent scan/learning/adaptive workflows are implemented; fixture-mode provider scan validation, composed workflow fixture coverage, live composed Codex-primary/Claude-challenger and Claude-primary/Codex-challenger validation, parallel reconciliation fixture coverage, route-equivalent `/screw:scan` provider-primary fixture validation, and one live Codex/Claude MLflow MoreFixes SSTI vulnerable/patched benchmark round trip are recorded; Codex CLI literal `/screw:*` autocomplete remains a tracked host-adapter gap, and additional provider adapters plus live parallel validation remain pending before Phase 5 closure |
...
now blocked as clean SQLi-quality evidence by mixed patched-source semantics:
review found likely residual-risk/incomplete-fix findings, ambiguous
second-order findings, and weak speculative patched reports in the same capped
