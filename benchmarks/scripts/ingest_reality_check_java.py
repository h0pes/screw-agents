"""Ingest the flawgarden/reality-check Java subset.
...
class RealityCheckJavaIngest(IngestBase):
    ...
    def extract_cases(self) -> list[BenchmarkCase]:
        repo_dir = self.download_dir / "repo"
        csv_path = repo_dir / "java" / "cves_db.csv"
...
def main() -> int:
    RealityCheckJavaIngest(root=Path("benchmarks")).run()
    return 0
