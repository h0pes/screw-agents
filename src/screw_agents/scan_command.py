...
def _split_csv(rest: str) -> list[str]:
    """Split a comma-separated value list and reject internal whitespace.
...
def parse_scope_spec(scope_text: str) -> ParsedScope:
    ...
    for tok in tokens:
        ...
        if ":" in tok:
            ...
            # Empty rest after `:` means the user wrote `domains: foo`
            # (whitespace after colon) and the shell-style tokenizer
            # already split the value off into a separate token. Raise
            # an actionable error instead of silently misclassifying
            # `foo` as a bare token (pre-audit Edit 11).
            if rest == "":
                raise ScopeResolutionError(
                    "Whitespace not allowed inside prefix-key values. "
                    "Use commas (no spaces) to separate names: "
                    f"'{prefix}:value1,value2' (no space after the colon)."
                )
            if prefix == "domains":
                domains_explicit.extend(_split_csv(rest))
            else:  # prefix == "agents"
                agents_explicit.extend(_split_csv(rest))
            continue
...
def resolve_scope(parsed: ParsedScope, registry: AgentRegistry) -> list[str]:
    ...
    for agent in parsed.agents_explicit:
        ...
        assert agent_def is not None  # checked above
        if (
